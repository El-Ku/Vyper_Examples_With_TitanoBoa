...
def splitter_impl():
    return boa.load("contracts/PaymentSplitter.vy") 

# deploy the factory contract and pass the splitter implementation address to it.
@pytest.fixture(scope="session")
def splitter_factory(splitter_impl):
    with boa.env.prank(ADMIN):
        return boa.load("contracts/PaymentSplitterFactory.vy", splitter_impl.address, FEE)
